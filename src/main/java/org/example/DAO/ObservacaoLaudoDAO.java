...
public class ObservacaoLaudoDAO extends ConexaoDB{
    ...
    public void deleteObservacaoLaudo(int id) {
        try (PreparedStatement preparedStatement = prepararSQL(DELETE_OBSERVACAO_LAUDO_SQL)) {
            preparedStatement.setInt(1, id);
            preparedStatement.executeUpdate();
        } catch (SQLException e) {
            printSQLException(e);
    ...
    public void updateObservacaoLaudo(ObservacaoLaudo entity) {
        try (PreparedStatement preparedStatement = prepararSQL(UPDATE_OBSERVACAO_LAUDO_SQL)) {
            preparedStatement.setString(1, entity.getDescricao());
            preparedStatement.setInt(2, entity.getReceitaOculosId());
